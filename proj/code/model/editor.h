...
/**
 * @brief Inserts a character from the remote peer at their cursor position
 * @param c Character to insert
 * @return EDITOR_OK on success; EDITOR_ERR_ALLOC_FAILED on allocation failure
 */
...
/**
 * @brief Deletes the character before the remote peer's cursor
 * @return EDITOR_OK on success; EDITOR_ERR_ALLOC_FAILED on allocation failure
 */
EditorResult editor_remote_delete_char();

/**
 * @brief Returns the remote peer's cursor row
 * @return Row index (0-based). -1 if no position has been received yet
 */
